...
/**
 * A simple class to render (draw) a Slice of blocks, using the block's ImageComponent. It will draw
 * a grid between the blocks
...
public class ImageGrid extends JComponent implements Printable, MouseListener {
                ...
                for (int x = 0; x < hzBlockNum; ++x) {
                    ...
                    if (this.markedBlock != null && this.markedBlock.x == x && this.markedBlock.y == y) {
                        ...
                        if (drawLeft && drawBottom) {
                            g.drawLine(left, bottom, left, bottom);
                        }
                        if (drawRight && drawTop) {
                            g.drawLine(right, top, right, top);
                        }
    ...
    /**
     * gets the index of the block inside the slice. Yeah I know, using Point for this makes not
     * much sense, but I'm not gonna make a new Datatype
    ...
    private Point getIndexOfBlockAtPoint(Point p) {
        ...
        if (x % doubleLineXRepetitionRate == doubleLineXRepetitionRate - 1) {
            return null;
        }
        ...
        if (x % lineXRepetitionRate == imgWidth) {
            return null;
        }
