...
public class SliceStack implements Iterable<Slice> {
    ...
    /**
     * Removes "Whitespace", eg air blocks, from all sides of the stack, so that the resulting stack
     * is the smallest cuboid without
     ...
     * Note that the images in the grids will be updated if the blocks change, so you have to call
     * this or Slice.getImages() or Block.getImage()
     * for every change
     *
     * @param zoom                   the current zoom value (min 1, since a 16x16 image/block is
     *                               small enough)
