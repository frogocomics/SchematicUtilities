...
/**
 * Vines can have different directions (walls they're attached to). A Vine block can be attached to
...
public class HugeMushroom extends MultiDirectionalBlock {
     ...
     * @param data the data containing information if it's a stem piece or a hood one (and which
     *             part of the hood)
     */
    public HugeMushroom(short id, byte data) {
        super(id, data);
        if (id != 99 && id != 100) {
            throw new IllegalArgumentException("Id not valid for huge mushrooms");
        }
     ...
     * @param directions the directions in which the hood texture is used (leave it null if this is
     *                   a stem or flesh block)
     */
    ...
    public void setData(byte data) {
        if (data < 0 || data > 10) {
            throw new IllegalArgumentException("block data out of data range: " + data);
        }
     ...
     *
     * @param directions valid directions are either any of N, E, S or W, or just TOP. Note that the
     *                   array should not have duplicates in it.
     */
    @Override
            ...
            for (Direction dir : this.directions) {
                switch (dir) {
                    ...
                    case UP:
                        ...
                        break; // no change
                    case NONE:
                        newDirections.add(Direction.NONE);
                        break; // no change
                    default:
                        // should never happen
    ...
    public synchronized BufferedImage getImage(double zoom) {
        ...
        for (Direction direction : this.directions) {
            switch (direction) {
                case N:
                    directionBitmap += (byte) 1;
                    break;
                case E:
                    directionBitmap += (byte) 2;
                    break;
                case S:
                    directionBitmap += (byte) 4;
                    break;
                case W:
                    directionBitmap += (byte) 8;
                    break;
                case UP:
                case NONE:
                    break; // do nothing
    ...
    public enum HugeMushroomBlockType {
        /**
         * a hood piece of the mushroom type
         */HOOD("hood piece"), /**
         * a stem piece
         */STEM("stem piece"), /**
         * an internal piece
        ...
        HugeMushroomBlockType(String name) {
            this.name = name;
        }
