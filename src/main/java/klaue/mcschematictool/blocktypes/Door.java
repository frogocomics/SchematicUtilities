...
/**
 * A door can be open or closed, have different directions, can be a top or bottom half and can be
 * wood or iron
 *
 * @author klaue
 */
public class Door extends DirectionalBlock {
    ...
    public Door(short id, byte data) {
        super(id, data);
        if (id != 64 && id != 71) {
            throw new IllegalArgumentException("ID is neither wood nor iron door: " + id);
        }
        this.doorType = (id == 64) ? DoorType.WOOD : DoorType.IRON;
        this.type = Type.DOOR;
        this.setData(data);
    }
    ...
    public Door(DoorType doorType, byte data) {
        super((short) ((doorType == DoorType.WOOD) ? 64 : 71), data);
        this.type = Type.DOOR;
     ...
     * @param isOpen       true if this door has swung counterclockwise around its hinge. (this is
     *                     actually closed for the "wrong"
     *                     half of a double door)
     * @param direction    the direction (Location of the hinge in the block), valid directions are
     *                     NE, SE, SW, NW
     */
    public Door(DoorType doorType, boolean isBottomHalf, boolean isOpen, Direction direction) {
        super((short) ((doorType == DoorType.WOOD) ? 64 : 71));
        this.isOpen = isOpen;
    ...
    /**
     * Returns true if this door is open (swung counterclockwise around its hinge - closed for the
     * "wrong" site of a double door)
     *
     * @return true if open
     */
    ...
    /**
     * Set to true if this door is open (swung counterclockwise around its hinge - closed for the
     * "wrong" site of a double door)
     *
    ...
    protected void setId(short id) {
        if (id == 64) {
            this.doorType = DoorType.WOOD;
        } else if (id == 71) {
            this.doorType = DoorType.IRON;
        } else {
