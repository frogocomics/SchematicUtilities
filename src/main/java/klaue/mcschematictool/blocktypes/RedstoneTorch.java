...
public class RedstoneTorch extends Torch {
    ...
    public RedstoneTorch(short id, byte data) {
        super(data);
        if (id != 75 && id != 76) {
            throw new IllegalArgumentException("not a valid redstone torch id: " + id);
        }
        this.setId(id);
        this.isOn = (id == 76);
     ...
     *
     * @param isOn      true if this redstone torch is burning
     * @param direction the direction (where the torch is pointing), valid directions are N, E, S, W
     *                  and None
     */
