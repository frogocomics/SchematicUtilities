...
public class Furnace extends DirectionalBlock {
    ...
    public Furnace(short id, byte data) {
        super(id, data);
        if (id != 61 && id != 62) {
            throw new IllegalArgumentException("illegal id for furnaces: " + id);
        }
     ...
     * initializes the furnace
     *
     * @param isOn      true if the furnace is burning (no sense in setting that manually since
     ...
     *
     * @param isOn true if the furnace is burning (no sense in setting that manually since contents
     *             can't be saved)
     */
