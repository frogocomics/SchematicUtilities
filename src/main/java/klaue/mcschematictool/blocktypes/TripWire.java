...
public class TripWire extends Block {
    ...
    // first booelan is line (true) or "cross" (false). when not called by Slice, line defaults to true and direction to E
     ...
     *
     * @param data 0 = not activated, bit 0x4 = whole tripwire activated, bit 0x1 = entity on this
     *             tripwire piece
     */
    ...
    /**
     * Sets this wires activated state. Note that this does not update the whole wire, just this
     * piece.
     *
    ...
    /**
     * @return true if this wire piece is the trigger for the wire activation, e.g. if there is
     * something on this wire
     */
    ...
    /**
     * Sets this wires trigger state. Trigger means there's something on this wire (this wire was
     * responsible for activating the rest of the wire)
     *
    ...
    public void setData(byte data) {
        if (data != 0 && data != 1 && data != 4 && data != 5) {
            throw new IllegalArgumentException("illegal data for Tripwires: " + data);
        }
    ...
    public String toString() {
        return super.toString() + " (" + (this.isActivated ? "activated" : "not activated") + ", " + (this.isTrigger ? "is trigger" : "not a trigger") + ")";
    }
    ...
    /**
     * Sets the type of the wire. Since the direction of a tripwire is not saved into the block
     * data,
     ...
     * @param direction the direction. For non-line wires, this is the direction where the wire is
     *                  in a 3x3 grid of wires. For example, a corner from
     *                  bottom to the right would be in the top-left corner of the grid and
     *                  therefore the direction would be north-west.
     *                  A cross-like intersection would be in the middle, which is direction "none".
     *                  For lines, this is the direction they are facing,
     *                  a line from bottom to top would be north or south, a line from left to right
     *                  would be west or east.<br>
     *                  Note that this direction is not the direction inside the minecraft world but
     *                  just used to determine the image. Therefore, north
     ...
     *
     * @param isWireInNorth true if there's another wire (or a power source) in the north of this
     *                      wire
     * @param isWireInEast  true if there's another wire (or a power source) in the east of this
     *                      wire
     * @param isWireInSouth true if there's another wire (or a power source) in the south of this
     *                      wire
     * @param isWireInWest  true if there's another wire (or a power source) in the west of this
     *                      wire
     */
    public void setWireType(boolean isWireInNorth, boolean isWireInEast, boolean isWireInSouth, boolean isWireInWest) {
        ...
        if ((isWireInWest || isWireInEast) && (!isWireInNorth && !isWireInSouth)) {
            this.setWireType(true, Direction.E);
        }
        if ((isWireInNorth || isWireInSouth) && (!isWireInWest && !isWireInEast)) {
            this.setWireType(true, Direction.S);
        }
        if (isWireInSouth && isWireInEast && !isWireInWest && !isWireInNorth) {
            this.setWireType(false, Direction.NW);
        }
        if (isWireInSouth && isWireInWest && !isWireInEast && !isWireInNorth) {
            this.setWireType(false, Direction.NE);
        }
        if (isWireInNorth && isWireInWest && !isWireInEast && !isWireInSouth) {
            this.setWireType(false, Direction.SE);
        }
        if (isWireInNorth && isWireInEast && !isWireInWest && !isWireInSouth) {
            this.setWireType(false, Direction.SW);
        }
        if (isWireInWest && isWireInSouth && isWireInEast && !isWireInNorth) {
            this.setWireType(false, Direction.N);
        }
        if (isWireInWest && isWireInSouth && isWireInNorth && !isWireInEast) {
            this.setWireType(false, Direction.E);
        }
        if (isWireInWest && isWireInNorth && isWireInEast && !isWireInSouth) {
            this.setWireType(false, Direction.S);
        }
        if (isWireInNorth && isWireInSouth && isWireInEast && !isWireInWest) {
            this.setWireType(false, Direction.W);
        }
    }
}
