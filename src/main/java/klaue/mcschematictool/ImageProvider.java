...
/**
 * This class reads the minecraft images and provides them trough getters and setters
...
public class ImageProvider {
    ...
    /**
     * Gets the image with the specified name. Make sure not to change the returned reference (only
     * copies of it) or the provider will get compromised
     *
    ...
    /**
     * Gets the image with the specified name from the additional image set.  Make sure not to
     * change the returned reference (only copies of it) or the provider will get compromised
     *
    ...
    /**
     * Gets the image with the specified name from the item image set.  Make sure not to change the
     * returned reference (only copies of it) or the provider will get compromised
     *
    ...
    /**
     * returns an array of BufferedImages that consists of 8x8 px representations of the given
     * (extended ASCII) text
     *
     * @param text  the text, has to be ext. ascii (code page 437)
     * @param color which color the text should be
     * @return 8x8px letter-images for the input string. note that minecraft cuts off the last two
     * pixels (which are empty for normal characters)
     */
    ...
    /**
     * Uses multiply to blend this image with the given color (same effect as a layer filled with a
     * color set to multiply in GIMP)
     * A new image instance will be returned, the original image will not be changed.
     *
    ...
    /**
     * Returns an image representing the given ID. Note that only images on zoom level 1 are
     * returned, and that additional overlays, like the direction,
     * are not added. The image returned is exactly as the one in minecrafts terrain.png or item.png
     * with the exception of stairs and slabs/double slabs
     * (there is no stair image in the files, it's just a normal block with one edge cut out, and
     * all slabs beside stone slabs also just use the normal
     * block image cut in half).
     * So if you, for example, need an image of a rail turned the right way to accomondate it's
     * direction, or a torch that has an arrow added for the
     ...
     *
     * @param id
     * @return an image representing the block. DO NOT CHANGE THIS IMAGE as all block instances will
     * use it
     */
     ...
     * Returns an image representing the given ID. Note that only images on zoom level 1 are
     * returned, and that additional overlays, like the direction,
     * are not added. The image returned is exactly as the one in minecrafts texture directory with
     * the exception of stairs and slabs/double slabs
     * (there is no stair image in the files, it's just a normal block with one edge cut out, and
     * all slabs beside stone slabs also just use the normal
     * block image cut in half).
     *
     * @param id
     * @param data the block data or -1 for default value of this block (like the image for white
     *             wool for a wool block)
     * @return an image representing the block. DO NOT CHANGE THIS IMAGE as all block instances will
     * use it
     */
    public final static synchronized BufferedImage getImageByBlockOrItemID(short id, byte data) {
        switch (id) {
            ...
            case 4:
                return ImageProvider.getImage("stonebrick"); // Cobblestone
            case 5:    // Wooden Plank
                switch (data) {
                    case 1:
            ...
            case 37:
                return ImageProvider.getImage("flower"); // Yellow flower
            case 38:
                return ImageProvider.getImage("rose");
            case 39:
            ...
            case 95:
                return ImageProvider.getAdditionalImage("chest"); // locked chest
            case 96:
            ...
            case 126:// wooden slab
                ...
                // it makes no sense to do that here so for this one block, we use the blocks getImage functionality
                byte woodenslabdata = (data < 0 || data > 3) ? 0 : data;
            ...
            case 144:
                return ImageProvider.getItemImage("skull_char"); // other heads determined trough tile entity
            case 145:
            ...
            case 342:
                return ImageProvider.getItemImage("minecartChest"); // Storage Minecart
            case 343:
                return ImageProvider.getItemImage("minecartFurnace"); // Powered Minecart
            case 344:
